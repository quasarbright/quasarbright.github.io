...
function addInputParams() {
  inputParams.forEach(inputParam => {
    let name,label,numtype,type,min,max,step    
    ({name, label, numtype, type, min, max, step} = inputParam)
    let def = inputParam.default
    let nodes = [
      `<label for="${name}"> ${label} </label>`
    ]
    if(type === INT_FIELD) {
      nodes.push(
        `<input name="${name}" id="${name}" type="number" min="${min}" max="${max}" value="${def}" step="${step}">`
      )
    } else {
      nodes.push(
        `<input class="slider" input name="${name}" id="${name}" type="range" min="${min}" max="${max}" value="${def}" step="${step}">`
      )
    }
    nodes.push("<hr>")
    nodes.forEach(node => {
      let inputs = document.getElementById("inputs")
      inputs.innerHTML += node
      inputs.innerHTML += "\n"
    })
    if(numtype === INT) {
      shaderData[name] = (gl,loc) => gl.uniform1i(loc, document.getElementById(name).value)
    } else {
      shaderData[name] = (gl, loc) => gl.uniform1f(loc, document.getElementById(name).value)
    }
  });
}
...
canvas.addEventListener('mousemove', (e) => {
  ...
  if (dragging) {
    ...
    let dcenter = deltaToCoord(dx, dy)
    centerx -= dcenter[0]
    centery -= dcenter[1]
    dragStart = curr
  }
