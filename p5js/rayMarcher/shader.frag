...
DistanceEstimation distanceEstimation(vec3 position) {
  return minDistanceEstimation(
    sphereDistance(position, Sphere(vec3(5,0,0), 1.0, diffuse(vec3(1,0,0)))),
    sphereDistance(position, Sphere(vec3(4,0,1), 1.0, diffuse(vec3(0,1,0))))
  );
}
...
HitInfo calculateRayCollision(Ray ray) {
  ...
  return HitInfo(false, DistanceEstimation(0.0, vec3(0), diffuse(vec3(0))), vec3(0));
}

vec3 trace(Ray ray) {
  HitInfo hitInfo = calculateRayCollision(ray);
  if(hitInfo.didHit) {
    return hitInfo.distanceEstimation.material.color;
  } else {
    return vec3(0);
  }
}
