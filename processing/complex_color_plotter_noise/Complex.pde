class Complex {
  ...
  Complex toPower(float exp) {
    float r2 = this.magSq();
    float theta = this.angle();
    float newR = pow(r2, exp/2.0);
    float newTheta = theta * exp;
    PVector newVec = PVector.fromAngle(newTheta).mult(newR);
    return new Complex(newVec);
  }
}
