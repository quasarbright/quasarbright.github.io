...
I have created a separate, extra sub-part explaining monads in the context of what we've been discussing. I decided to make it separate because it's pretty long and isn't directly focused on type inference. If you are not familiar with monads, or would like some review, read it [here](monads.md).
...
Our typing computations involve mutable state, local context, and possible errors. We want to abstract these behaviors away in our `Monad` instance so we can focus on the logic, rather than the book-keeping and error handling. 
...
```
Now lets define our `Checker` type and its instances:
```haskell
